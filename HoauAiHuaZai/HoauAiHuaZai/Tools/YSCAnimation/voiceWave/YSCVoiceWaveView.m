...
@interface YSCVoiceWaveView ()<CAAnimationDelegate>
...
- (CAShapeLayer *)selfViewMaskImage
{
    if (!_selfViewMaskImage) {
        _selfViewMaskImage = [[UIImageView alloc] init];
        _selfViewMaskImage.image = [UIImage imageNamed:@"maskImage"];
        _selfViewMaskImage.frame = self.bounds;
    }
    CAShapeLayer * shapeLayer = [[CAShapeLayer alloc]initWithLayer:_selfViewMaskImage.layer];
